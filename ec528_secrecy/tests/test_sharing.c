...
void test_bool_share_tables(Table *data, BShareTable* t1, BShareTable* t2,
  ...
  for (int i=0; i<data->numRows; i++) {
    for (int j=0; j<data->numCols; j++) {
      assert(t1->contents[i][2*j] == t3->contents[i][2*j+1]);
      assert(t1->contents[i][2*j+1] == t2->contents[i][2*j]);
      assert(t2->contents[i][2*j+1] == t3->contents[i][2*j]);
      assert(data->contents[i][j] == (t1->contents[i][2*j] ^
                                      t2->contents[i][2*j] ^
                                      t3->contents[i][2*j])
            );
    }
