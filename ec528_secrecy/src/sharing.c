...
void generate_bool_share_tables(Table *data, BShareTable *shares1,
  ...
  for (int i=0;i<data->numRows;i++) {
    ...
    for (int j=0;j<data->numCols;j++) {
       Data element = data->contents[i][j];
       BShare s1, s2, s3;
       generate_bool_share(element, &s1, &s2, &s3);
       // Populate first table
       shares1->contents[i][j+step] = s1;
       shares1->contents[i][j+step+1] = s2;
       // Populate second table
       shares2->contents[i][j+step] = s2;
       shares2->contents[i][j+step+1] = s3;
       // Populate third table
       shares3->contents[i][j+step] = s3;
       shares3->contents[i][j+step+1] = s1;
       step++;
    }
  }
}
...
void generate_int_share_tables(Table *data, AShareTable *shares1,
  ...
  for (int i=0;i<data->numRows;i++) {
    for (int j=0;j<data->numCols;j++) {
       ...
       // Populate first table
       shares1->contents[i][2*j] = s1;
       shares1->contents[i][2*j+1] = s2;
       // Populate second table
       shares2->contents[i][2*j] = s2;
       shares2->contents[i][2*j+1] = s3;
       // Populate third table
       shares3->contents[i][2*j] = s3;
       shares3->contents[i][2*j+1] = s1;
    }
  }
}
